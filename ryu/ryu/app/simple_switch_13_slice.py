...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #for classifier
        self.loaded_model = joblib.load('./ryu/ryu/app/ryu_customapp/models/b255v6 RandomForest choice_random=0.004 train_size=0.8 test_size=0.2 choice_split=3 choice_train=2 1630563027.216647.pkl')
        self.PacketCount=0
        ...
        for i in range(1, int(self.Host1half+1)):
            ...
            Host1half=i
            Host1halfMAC="00:00:00:00:00:"+str(hex(Host1half).lstrip("0x")).zfill(2)
        ...
        for i in range(self.Host1half+1, int(self.hostq+1)):
            ...
            PortS1right=i
            Host2half=i
            Host2halfMAC="00:00:00:00:00:"+str(hex(Host2half).lstrip("0x")).zfill(2)
    ...
    def _packet_in_handler(self, ev):
        ...
        if self.AllPacketInfo_ctrl == 1:
            ...
            self.logger.info('{0:>5} {1:>6} {2:>7} {3:>17} {4:>17} {5:<8}:{6:>5} {7:<8}:{8:>5}'.format(self.PacketCount, dpid, in_port,eth_src, eth_dst, ip_src, src_port, ip_dst, dst_port) )
            self.logger.info(" == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == =")
            ...
            try:
                ...
                if id_icmp.type == icmp.ICMP_ECHO_REQUEST:
                    if dpid>=self.slice_to_dpid[0] and dpid<=self.slice_to_dpid[self.SliceNum-1]:
                        ...
                        innerdelay=self.innerdelay[1] + self.innerdelay[id]
                        monitor=self.ping_monitor_timestamp[id]

                        req=self.ping_req_timestamp[id]
                        reqin=self.ping_reqin_timestamp[id]
                        latency_req = reqin  - req  - innerdelay
                        self.latency[1][self.dpid_to_slice[id]]=latency_req
                    ...
                    id=id_icmp.data.id
                    self.ping_rlyin_timestamp[id] = time.time()

                    innerdelay=self.innerdelay[1] + self.innerdelay[id]
                    monitor=self.ping_monitor_timestamp[id]

                    rly=self.ping_rly_timestamp[id]
                    rlyin=self.ping_rlyin_timestamp[id]                          
                    latency_rly = rlyin  - rly - innerdelay                    
                    self.latency[2][self.dpid_to_slice[id]]=latency_rly 

                    avg = (self.latency[1][self.dpid_to_slice[id]] +self.latency[2][self.dpid_to_slice[id]])/2
                    ...
                    if self.LatencyPrint_ctrl == 1:
                        ...
                        self.logger.info('{0:<4.3f}{1:<10} {2:<10.3f} {3:<10.3f}'.format(avg,bar,self.latency[1][self.dpid_to_slice[id]],self.latency[2][self.dpid_to_slice[id]] ))
                        self.logger.info(" == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == =")
                    
                    """
                    self.innerdelay[id]=0
                    self.ping_monitor_timestamp[id]=0
                    self.ping_req_timestamp[id]=0
                    self.ping_reqin_timestamp[id]=0
                    self.ping_rly_timestamp[id]=0
                    self.ping_rlyin_timestamp[id]=0
                    """
            except:
                pass
        ...
        if L3_ctrl == 1 and L4_ctrl == 1 and ICMP_ctrl == 1:
            try:
                ...
                if self.ClassPrint_ctrl == 1:
                    print(f'class={self.service_to_string[class_result]} count={self.ClassCount[class_result]}')   
            except:
                class_result=-1
                self.ClassCount[class_result]+=1
        ...
        if dpid in self.dstipv4_to_port and ipv4_dst in self.dstipv4_to_port[dpid]:
            ...
            if self.ActionPrint_ctrl == 1:
                self.logger.info(
                    "dst ip    s{0:<2}(out={1:>2})".format(dpid, out_port)
                )
            #match
            if self.FlowMatch_ctrl==1:
                if tcp_dst:
                    match = datapath.ofproto_parser.OFPMatch(eth_type=0x0800,ipv4_dst=ipv4_dst,ip_proto=ip_proto,tcp_dst=tcp_dst)
                elif udp_dst:
                    match = datapath.ofproto_parser.OFPMatch(eth_type=0x0800,ipv4_dst=ipv4_dst,ip_proto=ip_proto,udp_dst=udp_dst)
                else:
            ...
            self.add_flow(datapath=datapath, priority=1, match=match, actions=actions, hard_timeout=self.hard_timeout[class_result])
            ...
            if self.ActionPrint_ctrl == 1:
                self.logger.info(
                    "dst mac   s{0:<2}(out={1:>2})".format(dpid, out_port)
                )
            ...
            if self.ActionPrint_ctrl == 1:
                self.logger.info(
                    "src ip    s{0:<2}(out={1:>2})".format(dpid, out_port)
                )
            #match
            if self.FlowMatch_ctrl==1:
                if tcp_src:
                    match = datapath.ofproto_parser.OFPMatch(eth_type=0x0800,ipv4_src=ipv4_src,ip_proto=ip_proto,tcp_src=tcp_src)
                elif udp_src:
                    match = datapath.ofproto_parser.OFPMatch(eth_type=0x0800,ipv4_src=ipv4_src,ip_proto=ip_proto,udp_src=udp_src)
                else:
                    match = datapath.ofproto_parser.OFPMatch(eth_type=0x0800,ipv4_src=ipv4_src)
            else:
                match = datapath.ofproto_parser.OFPMatch(eth_type=0x0800,ipv4_src=ipv4_src)
            ...
            if self.ActionPrint_ctrl == 1:
                self.logger.info(
                    "src mac   s{0:<2}(out={1:>2})".format(dpid, out_port)
                )
            ...
            if self.ActionPrint_ctrl == 1:
                self.logger.info(
                    "src port  s{0:<2}(out={1:>2})".format(dpid, out_port)
                )
        ...
        if dpid == 1 or dpid == 2:
            for stat in sorted(body, key=attrgetter('port_no')):
                if stat.port_no<=2*self.SliceNum:
                    ...
                    prevtx=self.port_ctrl['prev_tx_bytes'][dpid][stat.port_no]
                    tx_bytes=currtx-prevtx

                    self.port_ctrl['prev_tx_bytes'][dpid][stat.port_no]=currtx
                    self.port_ctrl['Tx_flow'][dpid][stat.port_no]=tx_bytes
                    
                    currrx=stat.rx_bytes
                    prevrx=self.port_ctrl['prev_rx_bytes'][dpid][stat.port_no]
                    rx_bytes=currrx-prevrx

                    self.port_ctrl['prev_rx_bytes'][dpid][stat.port_no]=currrx
                    self.port_ctrl['Rx_flow'][dpid][stat.port_no]=rx_bytes

                    if self.UpdateBudget_ctrl==1:
                        self.bandfree[dpid]={i:self.bandwidth[i]-self.port_ctrl['Tx_flow'][dpid][self.slice_to_dstport[dpid][i]]-self.port_ctrl['Rx_flow'][dpid][self.slice_to_dstport[dpid][i]] for i in range(self.SliceNum)}
            ...
            if dpid == 1 or dpid == 2:
                ...
                for stat in sorted(body, key=attrgetter('port_no')):
                    ...
                    if dpid == 1 or dpid==2:
                        ...
                        if stat.port_no<=2*self.SliceNum:
                            bandload=self.port_ctrl['Rx_flow'][dpid][stat.port_no]+self.port_ctrl['Tx_flow'][dpid][stat.port_no]
                        else:
                            bandload=bandwidth
                    bar=str('')
                    ...
                    self.logger.info('%016x %8x %8d %8d %8d %8d %8d %8d %8.3f %8d %8d %s',
                                    ev.msg.datapath.id, stat.port_no,
                                    stat.rx_packets, stat.rx_bytes, self.port_ctrl['Rx_flow'][dpid][stat.port_no],
