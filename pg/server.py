...
from pkt_config.pkt_config import PKT_FILE_LIST,NUM_PKT,PKT_FILE_INTERVAL,CSV_OUTPUTPATH,GOGO_TIME,FIRST_TIME_SLEEP
...
def pc_flow(pkt,i):
    ...
    chk_socket=('10.0.0.'+str(slice_to_client(i)))+':'+str(src_port)+'-'+('10.0.0.'+str(slice_to_server(i)))+':'+str(dst_port)
...
def sendp_flow(i,sendp_count):
    ...
    if pkt_ctrl[i][pkt_socket]==0:
        try:
            flow[i][j][IP].src='10.0.0.'+str(slice_to_server(i))
            flow[i][j][IP].dst='10.0.0.'+str(slice_to_client(i))
